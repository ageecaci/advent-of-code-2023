...
class City:
    ...
    def explore_from(self, journey: Journey):
        ...
        valid_directions = get_valid_directions(visit.last_direction, visit.triple_direction)
...
@cache
def get_valid_directions(inbound_direction: Optional[str], direction_to_discard: Optional[str]) -> set[str]:
    valid_directions = set(directions)
    # we can only turn left or right: not go backwards
    ...
    # if told to discard (from a previous triple navigation) always do so
    valid_directions.discard(direction_to_discard)
