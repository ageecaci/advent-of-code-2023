...
class ModuleMap:
    ...
    def send_pulse(self) -> PulseCount:
        counts = {
            pulse_high: 0,
            pulse_low: 0,
        }
...
def main(props):
    ...
    total = PulseCount()
    count = 1000
    pulse_subtotals: list[dict[str, PulseCount]] = [total]
    state_indices: dict[Hashable, int] = {}
    current_state = module_map.get_state()
    state_indices[current_state] = 0
    for index in range(1, count + 1):
        pulses_sent = module_map.send_pulse()
        logger.debug('Button press %d resulted in pulses: %r', index, pulses_sent)
        total += pulses_sent
        pulse_subtotals.append(total)
        current_state = module_map.get_state()
        if current_state in state_indices:
            ...
            start_of_loop_subtotal = pulse_subtotals[start_of_loop_index]
            end_of_loop_subtotal = total
            loop_subtotal = end_of_loop_subtotal - start_of_loop_subtotal
            loop_length = end_of_loop_index - start_of_loop_index
            loops_required = (count - end_of_loop_index) // loop_length
            index_after_loops = end_of_loop_index + (loop_length * loops_required)
            index_offset = count - index_after_loops
            offset_pulse_subtotal = pulse_subtotals[index_offset] - start_of_loop_subtotal
            total = end_of_loop_subtotal + (loop_subtotal * loops_required) + offset_pulse_subtotal
            break
        else:
            state_indices[current_state] = index

    logger.info('Final pulse counts: %r', total)
    print(total.low * total.high)
